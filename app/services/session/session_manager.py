...
def refresh_session(session_id):
    ...
    current_count = int(session.get("message_count", 0))
...
def deactivate_session(session_id):
    session_key = f"session:{session_id}"
    redis_client.hset(session_key, "status", "inactive")


# =====================================
# Update Active Namespace
# =====================================
def update_active_namespace(session_id, namespace):
    session_key = f"session:{session_id}"
    redis_client.hset(session_key, "active_namespace", namespace)
...
def save_message(session_id, role, content):
    ...
    redis_client.rpush(history_key, json.dumps(message))
        ...
        sessions.append(
            {
                ...
                "active_namespace": metadata.get("active_namespace"),
